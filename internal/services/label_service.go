...
// AutoGenerateDeviceLabel automatically generates a simple barcode label for a new device
// This creates a basic label with just the device ID barcode
func (s *LabelService) AutoGenerateDeviceLabel(deviceID string) error {
	// Generate a simple barcode for the device ID
	// Size: 400x150 pixels (suitable for most label printers)
	barcodeImage, err := code128.Encode(deviceID)
	if err != nil {
		return fmt.Errorf("failed to encode barcode: %w", err)
	}

	// Scale barcode to desired size
	scaledBarcode, err := barcode.Scale(barcodeImage, 400, 150)
	if err != nil {
		return fmt.Errorf("failed to scale barcode: %w", err)
	}
	...
	labelWidth := 500
	labelHeight := 200
	labelImage := image.NewRGBA(image.Rect(0, 0, labelWidth, labelHeight))

	// Fill with white background
	draw.Draw(labelImage, labelImage.Bounds(), &image.Uniform{color.White}, image.Point{}, draw.Src)

	// Center the barcode
	offsetX := (labelWidth - scaledBarcode.Bounds().Dx()) / 2
	offsetY := (labelHeight - scaledBarcode.Bounds().Dy()) / 2
	draw.Draw(labelImage, image.Rect(offsetX, offsetY, offsetX+scaledBarcode.Bounds().Dx(), offsetY+scaledBarcode.Bounds().Dy()),
		scaledBarcode, image.Point{}, draw.Over)
