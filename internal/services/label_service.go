	...
	if elements, ok := labelData["elements"].([]map[string]interface{}); ok {
		for _, elem := range elements {
			...
			if elemType == "text" {
				...
				// Parse font size (default to 12pt if not specified)
				fontSize := 12.0
				if style != nil {
					if fs, ok := style["fontSize"].(float64); ok && fs > 0 {
						fontSize = fs
					}
				}
				...
				// Scale font size (basicfont is fixed, so we render larger and scale if needed)
				face := basicfont.Face7x13

				// Create a drawer for text
				point := fixed.Point26_6{
					X: fixed.Int26_6(x * 64),
					Y: fixed.Int26_6((y + int(fontSize)) * 64), // Baseline offset
				}
				...
				bounds := elemImg.Bounds()
				destRect := image.Rect(x, y, x+bounds.Dx(), y+bounds.Dy())
				draw.Draw(labelImage, destRect, elemImg, bounds.Min, draw.Over)
			}
		}
	}
