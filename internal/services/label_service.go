...
func NewLabelService() *LabelService {
	// Load default TrueType font
	fontPath := "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
	log.Printf("[LABEL INIT] Loading font from: %s", fontPath)

	fontData, err := ioutil.ReadFile(fontPath)
	if err != nil {
		log.Printf("[LABEL ERROR] Could not load TrueType font: %v", err)
		log.Printf("[LABEL ERROR] Text rendering will NOT work!")
		return &LabelService{}
	}

	font, err := truetype.Parse(fontData)
	if err != nil {
		log.Printf("[LABEL ERROR] Could not parse TrueType font: %v", err)
		log.Printf("[LABEL ERROR] Text rendering will NOT work!")
		return &LabelService{}
	}

	log.Printf("[LABEL INIT] Font loaded successfully: %d bytes, FUnitsPerEm=%d", len(fontData), font.FUnitsPerEm())
...
func (s *LabelService) AutoGenerateDeviceLabel(deviceID string) error {
	...
	log.Printf("[LABEL DEBUG] Label size in pixels: %dpx x %dpx (at 300 DPI)", labelWidthPx, labelHeightPx)
	...
	if elements, ok := labelData["elements"].([]map[string]interface{}); ok {
		for _, elem := range elements {
			...
			if elemType == "text" {
				// Render text element using TrueType fonts
				...
				// Parse font size from template
				// Template stores font_size as integer (e.g., 11), but JSON unmarshals as float64
				fontSize := 11.0 // default
				if style != nil {
					// Try parsing as float64 first (from JSON)
					if fs, ok := style["font_size"].(float64); ok && fs > 0 {
						fontSize = fs
					} else if fsInt, ok := style["font_size"].(int); ok && fsInt > 0 {
						fontSize = float64(fsInt)
					}
				}

				// Log template values for debugging
				log.Printf("[LABEL DEBUG] Text element: content='%s', x=%.1fmm, y=%.1fmm, fontSize=%.1fpt",
					content, elem["x"].(float64), elem["y"].(float64), fontSize)
				...
				if s.defaultFont != nil {
					...
					c.SetDPI(300) // 300 DPI for high quality
					c.SetFont(s.defaultFont)
					c.SetFontSize(fontSize) // Font size in points
					c.SetClip(labelImage.Bounds())
					c.SetDst(labelImage)
					c.SetSrc(image.NewUniform(textColor))
					c.SetHinting(font.HintingFull) // CRITICAL: Required for proper rendering

					// Calculate baseline position
					// Y in template is the TOP of the text box, we need to add the ascent to get baseline
					// freetype expects Y to be the baseline position
					// At 300 DPI: fontSize in points * 300/72 = pixels
					fontSizePixels := fontSize * 300.0 / 72.0

					// Baseline Y = element Y + font size (simple but effective approach)
					baselineY := y + int(fontSizePixels)

					log.Printf("[LABEL DEBUG] Rendering: x=%dpx, baselineY=%dpx, fontSizePx=%.1f, color=%v",
						x, baselineY, fontSizePixels, textColor)

					// Set drawing position (baseline)
					pt := freetype.Pt(x, baselineY)

					// Draw the text
					_, err := c.DrawString(content, pt)
					if err != nil {
						log.Printf("[LABEL ERROR] Error drawing text: %v", err)
					} else {
						log.Printf("[LABEL DEBUG] Text '%s' rendered successfully at (%d, %d)", content, x, baselineY)
					}
				} else {
					log.Printf("[LABEL ERROR] No default font available - text will not render!")
				}
