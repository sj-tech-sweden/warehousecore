...
func (s *LabelService) AutoGenerateDeviceLabel(deviceID string) error {
	...
	if elements, ok := labelData["elements"].([]map[string]interface{}); ok {
		for _, elem := range elements {
			...
			if elemType == "barcode" || elemType == "qrcode" || elemType == "image" {
				// Get image data
			...
			// Note: Text rendering would require a font library (like freetype)
			// For now, we skip text elements as they're complex to render server-side
		}
	}
